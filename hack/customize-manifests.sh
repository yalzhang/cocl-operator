...
"$YQ" eval-all -i '(select(.kind == "Namespace") | .metadata.name) = "confidential-clusters"' "${OPERATOR_DEPLOY_FILE}"
...
"$YQ" eval-all -i '(select(.kind == "Deployment") | .spec.template.spec.serviceAccountName) = "trusted-cluster-operator"' "${OPERATOR_DEPLOY_FILE}"
...
# 5. Patch Kind forwarding service namespaces
