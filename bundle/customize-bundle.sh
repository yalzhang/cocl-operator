...
# It transforms the upstream "trusted-cluster-operator" to downstream "cocl-operator".
...
echo "--> Customizing bundle manifests for cocl-operator..."

CSV_FILE="${BUNDLE_MANIFESTS}/trusted-cluster-operator.clusterserviceversion.yaml"

echo "-->  - Renaming CSV file..."
mv "${CSV_FILE}" "${BUNDLE_MANIFESTS}/cocl-operator.clusterserviceversion.yaml"
CSV_FILE="${BUNDLE_MANIFESTS}/cocl-operator.clusterserviceversion.yaml"

echo "-->  - Updating CSV metadata.name..."
VERSION=$(yq '.spec.version' "${CSV_FILE}")
yq -i ".metadata.name = \"cocl-operator.v${VERSION}\"" "${CSV_FILE}"
...
yq -i '.spec.install.spec.deployments[0].name = "cocl-operator"' "${CSV_FILE}"
yq -i '.spec.install.spec.deployments[0].spec.selector.matchLabels.app = "cocl-operator"' "${CSV_FILE}"
yq -i '.spec.install.spec.deployments[0].spec.template.metadata.labels.app = "cocl-operator"' "${CSV_FILE}"

echo "-->  - Updating container name..."
yq -i '.spec.install.spec.deployments[0].spec.template.spec.containers[0].name = "cocl-operator"' "${CSV_FILE}"

echo "-->  - Updating OPERATOR_NAME environment variable..."
yq -i '(.spec.install.spec.deployments[0].spec.template.spec.containers[0].env[] | select(.name == "OPERATOR_NAME")).value = "cocl-operator"' "${CSV_FILE}"
...
yq -i '.annotations."operators.operatorframework.io.bundle.package.v1" = "cocl-operator"' "${BUNDLE_METADATA}/annotations.yaml"
